 ...
 * connected directly via unique I2C addresses.  It implements the same
...
// I2C / TF‑Luna addresses
static const uint8_t  TFLUNA_ADDR_LENGTH = 0x10;
static const uint8_t  TFLUNA_ADDR_HEIGHT = 0x20;
static const uint8_t  TFLUNA_ADDR_WIDTH  = 0x30;
...
static bool tflInitOnAddr(uint8_t addr, const char* name, bool &flagOk) {
...
void setup() {
  ...
  tflInitOnAddr(TFLUNA_ADDR_LENGTH, "TF1(Length)", g_tfInit1Ok);
  tflInitOnAddr(TFLUNA_ADDR_HEIGHT, "TF2(Height)", g_tfInit2Ok);
  tflInitOnAddr(TFLUNA_ADDR_WIDTH, "TF3(Width)",  g_tfInit3Ok);
  Bridge.begin();
  delay(2000);
  bool start = false;
  while (!start) {
